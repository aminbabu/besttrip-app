...
const KTUsersUpdateLoginHistory = (function () {
  ...
  const loginHistorySignOutBtns = document.querySelectorAll(
    '[data-kt-users-login-session-table-filter="signout"]'
  );

  // Block login session
  const handleLoginHistoryStatus = () => {
    if (!loginHistoryBlockBtns) {
      return;
    }

    loginHistoryBlockBtns.addEventListener("click", (e) => {
      e.preventDefault();

      const url = loginHistoryBlockBtns.getAttribute("href");
  ...
  const handleLoginHistorySignOut = () => {
    if (!loginHistorySignOutBtns) {
      return;
    }

    loginHistorySignOutBtns.addEventListener("click", (e) => {
      e.preventDefault();

      const url = loginHistorySignOutBtns.getAttribute("href");
