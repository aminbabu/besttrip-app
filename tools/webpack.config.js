...
// paths
let rootPath = path.resolve(__dirname, "..").replaceAll(/\\/g, "/");
...
if (dev) {
  ...
  rootPath = path
    .resolve(__dirname, "../../../themes/" + theme + "/" + f + "html")
    .replaceAll(/\\/g, "/");
}

// get selected demo
let demo = getDemo() ?? "public";
...
function getEntryFiles() {
  const entries = {
    ...
    "css/style.bundle": [
      "./" +
        path.relative("./", srcPath).replaceAll(/\\/g, "/") +
        "/sass/style.scss",
      "./" +
        path.relative("./", srcPath).replaceAll(/\\/g, "/") +
        "/sass/plugins.scss",
  ...
  (glob.sync("./webpack/{plugins,js}/custom/**/*.+(js)") || [])
    .map((file) => {
  ...
  (
    ...
    .forEach((file) => {
      entries[file.replace(/.*js\/(.*?)\.js$/gi, "js/$1")] = "./" + file;
    });

  const entryPattern = path
    .join(srcPath, "js/widgets/**/*.js")
    .replaceAll(/\\/g, "/");
  // Widgets js
  ...
  if (dev) {
    entries["js/scripts.bundle"] =
      "./" +
      path.relative("./", rootPath).replaceAll(/\\/g, "/") +
    ...
    (
      glob.sync(
        "./" +
          path.relative("./", rootPath).replaceAll(/\\/g, "/") +
          "/tools/webpack/{plugins,js}/custom/**/*.+(js)"
      ) || []
    )
      .map((file) => {
        return file.replaceAll(/\\/g, "/");
    ...
    (
      glob.sync(
        path.relative("./", coreSrcPath).replaceAll(/\\/g, "/") +
          "/js/custom/**/!(_)*.js"
      ) || []
    )
      .map((file) => {
  ...
  if (
    args.indexOf("docs_html") !== -1 ||
    args.indexOf("docs_new_html") !== -1
  ) {
    entries["js/scripts.bundle"] =
      "./" +
      path.relative("./", rootPath).replaceAll(/\\/g, "/") +
...
function mainConfig() {
  return {
    ...
    resolve: {
      alias: {
        jquery: path
          .join(__dirname, "node_modules/jquery/src/jquery")
          .replaceAll(/\\/g, "/"),
        $: path
          .join(__dirname, "node_modules/jquery/src/jquery")
          .replaceAll(/\\/g, "/"),
        ...
        "@form-validation":
          (dev ? coreSrcPath : srcPath).replaceAll(/\\/g, "/") +
          "/plugins/@form-validation/cjs",
      },
      extensions: [".js", ".scss"],
      fallback: {
        util: false,
    ...
    module: {
      rules: [
        ...
        },
        {
          test: /\.(gif|png|jpe?g)$/,
          include: [
            path.resolve(__dirname, "node_modules").replaceAll(/\\/g, "/"),
          ],
          use: [
            {
              loader: "file-loader",
              options: {
                ...
                publicPath: (url, resourcePath, context) => {
                  return path.basename(url).replaceAll(/\\/g, "/");
                },
                outputPath: (url, resourcePath, context) => {
                  var plugin = url.match(/node_modules\/(.*?)\//i);
                  if (plugin) {
                    return `plugins/custom/${plugin[1]}/${path
                      .basename(url)
                      .replaceAll(/\\/g, "/")}`;
                  }
                  return url;
  ...
  let options = [
    ...
    },
    {
      ...
      from:
        ...
        "/tinymce/skins",
      to:
        assetDistPath.replaceAll(/\\/g, "/") + "/plugins/custom/tinymce/skins",
    },
    {
      ...
      from:
        ...
        "/tinymce/plugins",
      to:
        assetDistPath.replaceAll(/\\/g, "/") +
        "/plugins/custom/tinymce/plugins",
    },
  ];

  if (
    fs.existsSync(coreSrcPath.replaceAll(/\\/g, "/") + "/media/plugins/jstree")
  ) {
    options.push({
      // copy jstree image
      from: coreSrcPath.replaceAll(/\\/g, "/") + "/media/plugins/jstree",
      to: assetDistPath.replaceAll(/\\/g, "/") + "/plugins/custom/jstree",
      force: true,
    });
  }
    ...
    new MergeIntoSingle({
      files: {
        "plugins/custom/datatables/datatables.bundle.js": [
          ...
          "../public/src/js/vendors/plugins/datatables.init.js",
...
function getDemo() {
  const key = Object.keys(process.env).find((element) =>
    element.match(/npm_config_(demo\d+)$/)
  );
